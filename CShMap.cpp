...
template <typename Key, typename Value> class CSHMap {
public:
    CSHMap() : rbTree() {}
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (command == "at") {
            iss >> key;
            try {
                std::cout << CSHMap.at(key) << std::endl;
