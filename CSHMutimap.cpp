...
int main() {
    CSHMultiMap<int, int> myCSHMultiMap;
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (command == "insert") {
            iss >> key >> value;
            myCSHMultiMap.insert(key, value);
        }
        
        if (command == "remove") {
            iss >> key >> value;
            myCSHMultiMap.remove(key, value);
        }
        
        if (command == "remove_all") {
            iss >> key;
            myCSHMultiMap.remove(key);
        }
        
        if (command == "size") {
            std::cout << myCSHMultiMap.getSize()  << std::endl;
        }
        
        if (command == "empty") {
            std::cout << (myCSHMultiMap.empty() ? "true" : "false") << std::endl;
        }
        
        if (command == "at") {
            iss >> key;
            auto valueList = myCSHMultiMap.at(key);
            if (valueList) {
