...
template<typename Key, typename Value>class CSHRedBlackTree
{
    class Node
    {
    public:
        ...
        Node* right;
        Node* parent;
        Node(const Key& k, const Value& value, Color color, Node* p = nullptr)
            : key(k), value(value), color(color), left(nullptr), right(nullptr), parent(p) {}
...
private:
    ...
    Node* lookUp(Key key)
    {
        ...
        return current;//这个地方应该是nullptr
    }

    //右旋函数
    void rightRotate(Node* node)
    {
        Node* l_son = node->left;//获得当前节点的左子节点
        // 当前节点的左子树变成左子节点的右子树
    ...
    {
        Node* r_son = node->right;//获得当前节点的右子节点

        node->right = r_son->left;// 当前节点的右子树变成右子节点的左子树
...
public:
    // 构造函数
    CSHRedBlackTree() : root(nullptr), size(0), Nil(new Node()) {
...
int main() {
    // 创建红黑树实例
    CSHRedBlackTree<int, int> rbTree;
