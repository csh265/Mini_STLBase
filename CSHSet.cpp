...
private:
        ...
        while (node != root) {
            ...
            if (node == node->parent->left) {
                ...
                if (getColor(sibling) == Color::RED) {
                    // 重新着色兄弟节点和父节点，并进行左旋
                    CSHSetColor(sibling, Color::BLACK);
                    CSHSetColor(node->parent, Color::RED);
                ...
                if (getColor(sibling->left) == Color::BLACK &&
                    ...
                    if (getColor(sibling->right) == Color::BLACK) {
                        ...
                        CSHSetColor(sibling->left, Color::BLACK);
                        CSHSetColor(sibling, Color::RED);
                        rightRotate(sibling);
                        // 旋转后更新兄弟节点
                        sibling = node->parent->right;
                    ...
                    // 情况4：兄弟节点的右子节点是红色
                    CSHSetColor(sibling, getColor(node->parent));
                    CSHSetColor(node->parent, Color::BLACK);
                    CSHSetColor(sibling->right, Color::BLACK);
                    leftRotate(node->parent);
                    // 移动到根节点结束
                    node = root;
                }
    ...
    void CSHSetColor(Node *node, Color color) {
        if (node == nullptr) {
            return;
        }
        node->color = color;
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (command == "insert") {
            iss >> element;
            myCSHSet.insert(element);
        }
        
        if (command == "earse") {
            iss >> element;
            myCSHSet.erase(element);
        }
        
        if (command == "contains") {
            iss >> element;
            std::cout << (myCSHSet.contains(element) ? "true" : "false") << std::endl;
        }
        
        if (command == "size") {
            std::cout << myCSHSet.size() << std::endl;
        }
        
        if (command == "empty") {
            std::cout << (myCSHSet.empty() ? "true" : "false") << std::endl;
        }
    }
    return 0;
}
